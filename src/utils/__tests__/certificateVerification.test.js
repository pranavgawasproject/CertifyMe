...
describe('Certificate Verification Utilities', () => {
  ...
  describe('calculateCertifyMeBulkCertificateGenerationQuotaIndex', () => {
    it('calculates quota utilization and batch approval correctly', () => {
      const res = calculateCertifyMeBulkCertificateGenerationQuotaIndex({
      ...
      expect(res.valid).toBe(true);
      expect(res.remainingQuota).toBe(150);
      expect(res.isBatchAllowed).toBe(true);
      expect(res.quotaUtilizationPct).toBe(70);
      expect(res.quotaTier).toBe('SUFFICIENT_QUOTA_AVAILABLE');
    });

    it('returns error for invalid non-positive quota input', () => {
      const inv = calculateCertifyMeBulkCertificateGenerationQuotaIndex({ monthlyGenerationQuota: 0 });
      expect(inv.valid).toBe(false);
      expect(inv.error).toBe('Monthly generation quota must be a positive number');
    });
  });

  describe('calculateCertificateTemplateDesignScore', () => {
    it('calculates PROFESSIONAL_TEMPLATE tier for optimal contrast and branding', () => {
      const res = calculateCertificateTemplateDesignScore({
        fontContrastRatio: 4.5,
        hasLogoImage: true,
      ...
      expect(res.valid).toBe(true);
      expect(res.designScore).toBe(100);
      expect(res.designTier).toBe('PROFESSIONAL_TEMPLATE');
      expect(res.recommendation).toContain('Professional certificate template design');
    });

    it('returns error for non-positive font contrast ratio', () => {
  ...
  describe('calculateCertificateVerificationHashAndRevocationAudit', () => {
    it('verifies authentic valid certificate hash and signature', () => {
      const res = calculateCertificateVerificationHashAndRevocationAudit({
        certificateHashId: 'CERT_HASH_123',
        revocationList: [],
        issuerSignatureValid: true
      });
      expect(res.valid).toBe(true);
      ...
      expect(res.authenticityScore).toBe(100);
      expect(res.verificationStatusTier).toBe('VERIFIED_AUTHENTIC');
      expect(res.recommendation).toContain('Certificate verified authentic');
    });

    it('flags REVOKED_CREDENTIAL when hash is present in revocation list', () => {
      const res = calculateCertificateVerificationHashAndRevocationAudit({
        certificateHashId: 'CERT_HASH_123',
        revocationList: ['cert_hash_123']
      });
      expect(res.valid).toBe(true);
      expect(res.isRevoked).toBe(true);
      expect(res.authenticityScore).toBe(0);
      expect(res.verificationStatusTier).toBe('REVOKED_CREDENTIAL');
    });

    it('returns error for empty certificate hash string', () => {
      const inv = calculateCertificateVerificationHashAndRevocationAudit({ certificateHashId: ' ' });
      expect(inv.valid).toBe(false);
      expect(inv.error).toBe('Certificate hash ID must be a non-empty string');
    });
  });

  describe('calculateCertificateTrustTierAndAuditScore', () => {
    it('calculates trusted audit score for verified issuer and signed certificate', () => {
      const res = calculateCertificateTrustTierAndAuditScore({
        isIssuerVerified: true,
      ...
      expect(res.trustTier).toBe('VERIFIED_TRUSTED_CREDENTIAL');
      expect(res.recommendation).toContain('Highest trust tier credential verified');
    });

    it('returns error for non-number daysUntilExpiration', () => {
      const inv = calculateCertificateTrustTierAndAuditScore({ daysUntilExpiration: 'invalid' });
      expect(inv.valid).toBe(false);
      expect(inv.error).toBe('Days until expiration must be a number');
    });
  });

  describe('calculateEnterpriseCertificateRevocationAudit', () => {
    it('calculates full enterprise compliance for clean batch', () => {
      const res = calculateEnterpriseCertificateRevocationAudit({
        totalCertificatesInBatch: 100,
        revokedCertificatesCount: 0,
        expiredCertificatesCount: 0,
        verifiedDomainIssuersCount: 100
      });
      expect(res.valid).toBe(true);
      expect(res.integrityPercentage).toBe(100);
      expect(res.auditScore).toBe(100);
      expect(res.complianceTier).toBe('FULL_ENTERPRISE_COMPLIANCE');
    });

    it('returns error for invalid non-positive batch size', () => {
      const inv = calculateEnterpriseCertificateRevocationAudit({ totalCertificatesInBatch: 0 });
      expect(inv.valid).toBe(false);
      expect(inv.error).toBe('Total certificates in batch must be a positive number');
    });
  });
});
