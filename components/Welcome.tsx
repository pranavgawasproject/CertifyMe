...
interface WelcomeProps {
  /** Optional initial template id from the ?template= query param. */
  initialTemplateId?: string;
}

export default function Welcome({ initialTemplateId }: WelcomeProps) {
  ...
  const [issuer, setIssuer] = useState('CertifyMe');
  ...
  // Hydrate from sessionStorage / query param after mount (SSR-safe).
  useEffect(() => {
    ...
    setRecipientName(getStorageItem('cert_recipientName') || '');
    setEvent(getStorageItem('cert_event') || '');
    setDate(getStorageItem('cert_date') || today);
    setIssuer(getStorageItem('cert_issuer') || 'CertifyMe');
    setSignature(getStorageItem('cert_signature') || '');
    setLogoUrl(getStorageItem('cert_logoUrl') || '');
    setSelectedTemplate(initialTemplateId || getStorageItem('cert_template') || 'classic-gold');
    setHydrated(true);
  }, [initialTemplateId]);

  // Persist any field changes
  useEffect(() => {
    if (!hydrated) return;
  ...
  }, [recipientName, event, date, issuer, signature, logoUrl, selectedTemplate, hydrated]);

  const categories = ['All', ...Array.from(new Set(TEMPLATES.map((t) => t.category)))];
  const filteredTemplates =
    activeCategory === 'All' ? TEMPLATES : TEMPLATES.filter((t) => t.category === activeCategory);

  const previewData: CertificateData = { recipientName, event, date, issuer, signature };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validateForm()) {
      // Cert data is already persisted to sessionStorage via the effect above.
  ...
  return (
    <div>
      ...
      <section className="relative pt-12 pb-8 px-4 sm:px-6">
        <div className="container mx-auto max-w-6xl text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight mb-4">
            <span className="font-display text-[#C9A24B]">Design a certificate</span>
            <br />
            <span className="text-white">worth showing off.</span>
          </h1>
          <p className="text-slate-300 text-base sm:text-lg max-w-2xl mx-auto mb-6">
            Pick a template, fill in the details, watch the preview update live, and download a
            high-resolution PNG — all in one place.
          </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-6">
            ...
            >
              ...
              <p className="text-xs text-slate-400 mb-5">
                All fields sync to the preview in real time.
              </p>
              ...
              <div className="mb-4">
                ...
                />
                {errors.event && (
                  <p className="text-red-300 text-xs mt-1.5">⚠ {errors.event}</p>
                )}
              </div>
              ...
              <div className="mb-6">
                <label className="block text-slate-200 font-medium mb-1.5 text-sm">
                  Organization logo{' '}
                  <span className="text-slate-500 font-normal">(optional, &lt;500KB)</span>
                </label>
                {logoUrl ? (
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between mb-6 gap-3">
            <div>
              ...
              <p className="text-slate-400 text-sm mt-1">
                20 hand-crafted designs across 7 styles. Click any one to preview.
              </p>
            </div>
      ...
      <section className="px-4 sm:px-6 pb-20">
        <div className="container mx-auto max-w-5xl">
          <h2 className="text-2xl sm:text-3xl font-bold text-white text-center mb-8">
            Built for every kind of recognition
          </h2>
      ...
      <footer className="border-t border-white/10 py-6 text-center text-xs text-slate-500">
        Built with Next.js + Tailwind. Crafted by the CertifyMe community.
      </footer>
    </div>
  );
}
