...
export default function Certificate() {
  ...
  // Read state from sessionStorage (the source used react-router's location.state; in
  // App Router we rely on sessionStorage exclusively since the Welcome page persists
  // every field change there).
  ...
  useEffect(() => {
    const rName = getStorageItem('cert_recipientName') || '';
    const ev = getStorageItem('cert_event') || '';
    const dt = getStorageItem('cert_date') || '';
    const iss = getStorageItem('cert_issuer') || '';
    const sig = getStorageItem('cert_signature') || '';
    const logo = getStorageItem('cert_logoUrl') || '';
    const tpl = getStorageItem('cert_template') || 'classic-gold';
  ...
  if (!hydrated || !recipientName || !event) {
    // Render a minimal placeholder while client state hydrates / before redirect kicks in.
    return (
      <div>
        <Navbar />
  ...
  const switchTemplate = (tplId: string) => {
    ...
    <div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 sm:px-6 py-8">
        <div className="max-w-4xl mx-auto mb-8 animate-fade-in">
          <div className="bg-[#C9A24B]/10 backdrop-blur-xl p-6 sm:p-8 rounded-md border border-white/10 text-center">
            ...
            <p className="text-slate-300 text-base sm:text-lg">
              ...
              <span className="font-semibold text-[#C9A24B]">{event}</span> is ready.
            </p>
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              <span className="bg-amber-500/20 text-amber-200 px-3 py-1 rounded-full text-xs">
                Template: {currentTemplate.name}
              </span>
              <span className="bg-emerald-500/20 text-emerald-200 px-3 py-1 rounded-full text-xs">
                ✓ Ready to download
              </span>
              <span className="bg-[#C9A24B]/20 text-[#DAB86A] px-3 py-1 rounded-full text-xs">
                📐 A4 Landscape · 2x export
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto mb-8 animate-scale-in">
          ...
          >
            <CertificatePreview templateId={selectedTemplate} data={data} logoUrl={logoUrl} />
          </div>
        </div>
          ...
          <div className="max-w-3xl mx-auto mb-6 bg-white/[0.04] border border-white/10 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex-1 min-w-0">
              <div className="text-xs text-slate-400 mb-1">
                🔗 Shareable link to this certificate
              </div>
              <input
                readOnly
                value={shareLink}
          ...
          <div className="max-w-5xl mx-auto mb-12 bg-white/[0.04] border border-white/10 rounded-md p-5 animate-fade-in">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-semibold text-sm">
                Switch template (keeps your details)
              </h3>
              <button
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div
              key={tip.title}
              className="bg-white/[0.04] backdrop-blur border border-white/10 p-5 rounded-xl text-center"
            >
