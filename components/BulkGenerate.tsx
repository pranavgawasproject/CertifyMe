...
export default function BulkGenerate() {
  const [rows, setRows] = useState<CsvBatchRow[]>([]);
  ...
  const handleFile = (file: File) => {
    ...
    // Dynamic import — papaparse is a browser-heavy library, only load when needed.
    import('papaparse').then((Papa) => {
      Papa.default.parse(file, {
        header: true,
        skipEmptyLines: true,
        complete: (results: { data?: Record<string, unknown>[]; errors?: unknown[] }) => {
        ...
        error: (err: Error) => setError(`Failed to parse CSV: ${err.message}`),
      });
    });
  };
  ...
  const removeRow = (idx: number) => setRows(rows.filter((r) => r._idx !== idx));
  ...
  const downloadAllZip = async () => {
    ...
    try {
      ...
      setError(`Bulk export failed: ${msg}`);
    } finally {
      ...
      <section className="px-4 sm:px-6 pt-12 pb-8">
        <div className="container mx-auto max-w-5xl text-center">
          <h1 className="text-4xl sm:text-5xl font-bold mb-4">
            <span className="font-display text-[#C9A24B]">Bulk Certificate Generator</span>
          </h1>
          <p className="text-slate-300 text-base sm:text-lg max-w-2xl mx-auto">
            Upload a CSV with your recipients. Get a ZIP of personalized PNG certificates in one
            click. No sign-up, no limits, runs entirely in your browser.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-3xl">
          ...
          >
            ...
            <p className="text-slate-400 text-xs mb-4">
              Required columns: <code className="text-[#C9A24B]">recipientName</code>,{' '}
              <code className="text-[#C9A24B]">event</code>. Optional: date, issuer, signature,
              template
            </p>
            <input
              ref={fileInputRef}
          ...
          <div className="container mx-auto max-w-6xl">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-slate-400 text-xs">
                    Preview below · click any to remove
                  </span>
                  <span className="bg-[#C9A24B]/20 text-[#DAB86A] text-[11px] px-2 py-0.5 rounded-full">
                    ⚡ Est. ~
                    {calculateCertificateBatchIssuanceSummary(rows as unknown as never[]).estimatedProcessingTimeSeconds}s
                    export
                  </span>
                </div>
              </div>
            ...
            {(isZipping || isPdfing) && (
              <div className="w-full bg-slate-800 rounded-full h-2 mb-6 overflow-hidden">
                <div
                  className="bg-[#8C2F39] h-full transition-all"
        ...
        <div className="container mx-auto max-w-3xl">
          <div className="bg-white/[0.04] border border-white/10 rounded-md p-6">
                  ...
                  {(
                    ...
                    <tr key={col}>
                      <td className="py-2 pr-4">
                        <code className="text-[#C9A24B]">{col}</code>
                      </td>
                      <td className="py-2 pr-4">
                        {req === 'Yes' ? <span className="text-red-400">{req}</span> : req}
                      </td>
            ...
            <p className="text-slate-400 text-xs mt-4">
              💡 Tip: To find valid template IDs, visit the{' '}
              <a href="/templates" className="text-[#C9A24B] underline">
                templates page
              </a>
              . Examples: <code className="text-[#C9A24B]">classic-gold</code>,{' '}
              <code className="text-[#C9A24B]">royal-blue</code>,{' '}
              <code className="text-[#C9A24B]">tech-neon</code>,{' '}
              <code className="text-[#C9A24B]">art-deco</code>.
            </p>
          </div>
        </div>
      </section>
